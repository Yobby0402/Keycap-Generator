    ...
    def _create_default_config_for_type(self, type_id: str) -> Optional[BatchEditConfig]:
        ...
        config = BatchEditConfig(
            ...
            geometry=KeycapGeometry(
                ...
                stem_type=getattr(dg, 'stem_type', 'MX'),
                ...
                stabilizer_cross_width=getattr(dg, 'stabilizer_cross_width', 1.3),
                stabilizer_cross_length=getattr(dg, 'stabilizer_cross_length', 4.0),
                stabilizer_cylinder_diameter=getattr(dg, 'stabilizer_cylinder_diameter', 4.0),
                stabilizer_depth=getattr(dg, 'stabilizer_depth', 5.0),
    ...
    def _update_3d_preview(self, config: BatchEditConfig):
        ...
        try:
            ...
            geometry = KeycapGeometry(
                ...
                stabilizer_cross_width=getattr(config.geometry, 'stabilizer_cross_width', 1.3),
                stabilizer_cross_length=getattr(config.geometry, 'stabilizer_cross_length', 4.0),
                stabilizer_cylinder_diameter=getattr(config.geometry, 'stabilizer_cylinder_diameter', 4.0),
                stabilizer_depth=getattr(config.geometry, 'stabilizer_depth', 5.0),
